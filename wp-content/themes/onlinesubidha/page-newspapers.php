...
<div class='container'>
  <div class='panel panel-primary dialog-panel'>
    <div class='panel-heading'><h5>News</h5></div>
    <?php
    $taxonomy = 'newspaper_cat';
    ...
    foreach($categories as $category) {
      if ( 0 == $category->category_parent ) {
        ...
        <table class="table table-striped table-bordered links">
          ...
          <tbody>
          <?php 
          $myquery['tax_query'] = array( array('taxonomy' => $taxonomy,'status'=>'publish','field' => 'id','terms' => $category->term_id,'include_children' => false,'operator' => 'IN'));
          query_posts($myquery);
          if(have_posts()){ 
            while(have_posts()){ 
              the_post(); 
